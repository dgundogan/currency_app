...
class CurrencyResource(Resource):
    def get(self):
        ...
        if not date:
            return {"status": "error", 'message': 'No date parameter provided'}, 400
    ...
    def post(self):
        ...
        if not data:
            return {"status": "error", 'message': 'Cannot access the source'}, 500
        ...
        for currency, rate in data["rates"].items():
            ...
            exist = Currency.query.filter_by(date=date, base=data["base"], currency=currency).first()
            if not exist:
                currrency_rate = Currency(date=date, base=data["base"], currency=currency, rate=rate)
                db.session.add(currrency_rate)
                record_count+=1
        
        db.session.commit()

        return Response("{} records is inserted".format(record_count),status=201)